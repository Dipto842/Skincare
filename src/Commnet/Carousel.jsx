  ...
  return (
    <div className="bg-[#FAFAF0] p-8 rounded-lg mt-20">
      <div className="lg:flex  items-center justify-between mb-32 ">
        ...
        <div className="flex buttonwidth lg:gap-10 buton">

          <button  ref={prevRef} className="bg-white w-[80px] h-[80px] hover:bg-[#2D3B36] hover:text-white button3 lg:text-6xl lg:pl-[35px] text-black border border-black pb-4 rounded-full">←</button>
          <button ref={nextRef} className="bg-white  hover:bg-[#2D3B36] hover:text-white button2 w-[80px] h-[80px] lg:text-6xl lg:pr-[65px] text-black pb-3 rounded-full  border border-black">→</button>
        </div>
      </div>

{
  products.length === 0
    ? <div className="text-center text-3xl font-bold mt-20">Data Is  Loading<span><BeatLoader /></span></div>
    : null
}
       ...
       <div className="overflow-hidden">
         {products.map(product => (
           ...
           <SwiperSlide  key={product._id}>
             <Link to={`/deteil/${product._id}`} >
            
            <div className="bg-white w-full card lg:h-[600px] rounded-xl overflow-hidden shadow-md z-0 cursor-pointer ">
              <img src={product.image} alt={product.name} className="w-full imges5 h-[600px] object-cover" />
            ...
            <div className='flex justify-between  flex-row-reverse items-center -mt-2  text-center gap-4 '>
               ...
               {
                product.stock==='outstock'?  <div disabled={true} className="lg:w-[80px]  btn hover:bg-lime-600 imgesbox imges4 lg:h-[80px] bg-[#2D3B361A] mr-5 mt-4 text-center flex justify-center items-center">
              
             
                <p  className='  imges4 text-[30px]  text-[#2D3B36] '> <FaShoppingCart /></p>
             

                </div> :  <div className="lg:w-[80px] btn hover:bg-lime-600 imgesbox imges4 lg:h-[80px] bg-[#2D3B361A] mr-5 mt-4 text-center flex justify-center items-center">
              
             
                <p onClick={()=>hendelcard(product._id)} className='  imges4 text-[30px]  text-[#2D3B36] '> <FaShoppingCart /></p>
